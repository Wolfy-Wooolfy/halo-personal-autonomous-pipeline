...
6. Exploration Artifacts (analysis-only authority)
...
# EXPLORATION ARTIFACTS (PRE-DECISION ANALYSIS)

Exploration artifacts are generated by the Design Exploration Module.

They exist outside stage authority and serve analytical purposes only.

Exploration artifacts MUST NOT:

- advance stages
- alter execution state
- modify repository files
- override Decision Gate authority

They exist only to assist deterministic decision escalation.
...
Path:
artifacts/exploration/option_matrix.md

Type:
Exploration Artifact (analysis-only authority)

Purpose:
Structured comparison of alternative architectural or structural options.
...
Narrative allowed:
YES (analysis context only)

This artifact MUST NOT be used to bypass Decision Gate.

---

## E2 — proposal_analysis.json

Path:
artifacts/exploration/proposal_analysis.json

Type:
Exploration Artifact

Purpose:
Machine-readable analysis of proposed system modification.
...
## E3 — recommendation_report.json

Path:
artifacts/exploration/recommendation_report.json

Type:
Exploration Artifact

Purpose:
Provide structured recommendation for Decision Gate packet generation.
...
Decision Gate MAY reference this artifact
but MUST NOT treat it as authoritative.

---

## E4 — implementation_option_matrix.md

Path:
artifacts/exploration/implementation_option_matrix.md

Type:
Exploration Artifact

Purpose:
Compare implementation strategies when multiple valid code generation paths exist.

Mandatory Content:

- implementation_option_id
- complexity_estimate
...
Artifacts MUST NOT:
- Grant execution authority when classified as Exploration Artifact
